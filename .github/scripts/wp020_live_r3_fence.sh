...
AUTH_MARKER="FRESH_OWNER_AUTHORIZED_R3: ${WP020_LIVE_EXECUTION_ID} @ ${GITHUB_SHA}"
...
if ! printf '%s\n' "${COMMENTS}" | grep -Fx "${AUTH_MARKER}" >/dev/null; then
  echo "STOP: exact fresh Owner R3 paid authorization marker missing for current main" >&2
  exit 1
fi
if printf '%s\n' "${COMMENTS}" | grep -Fx "${FENCE_MARKER}" >/dev/null; then
  echo "STOP: R3 one-shot execution fence already consumed" >&2
  exit 1
fi
if printf '%s\n' "${COMMENTS}" | grep -F "${PASS_MARKER}" >/dev/null || printf '%s\n' "${COMMENTS}" | grep -F "${STOP_MARKER}" >/dev/null; then
  echo "STOP: R3 execution identity already has terminal evidence" >&2
  exit 1
fi

case "${MODE}" in
  check)
    echo "Fresh Owner authorization present; R3 identity remains unused"
    ;;
  consume)
    gh issue comment "${ISSUE_NUMBER}" --body "${FENCE_MARKER}"
    echo "R3 one-shot execution fence consumed"
    ;;
  *)
