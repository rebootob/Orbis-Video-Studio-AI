"""P4-WP020-LIVE fresh authorization preflight.

This script MUST NOT call any external AI provider. It validates only credential
presence, production adapter configuration, pricing/budget guards, DB migration
runtime, and S3-compatible object storage against the ephemeral UAT environment.
...
def main() -> None:
    ...
    # Locked provider/model/retry contract. Never expose credential values.
    ...
    # Generic pricing registry must be explicit and non-UNKNOWN for chargeable
    # paths that depend on it. Audio has its own exact adapter estimator.
    ProviderPricingService.reset()
    openai_cost, curr, status = ProviderPricingService.estimate_cost(
        "openai",
        "STORY_GENERATION",
        "gpt-4o",
        {"prompt_tokens": 500, "completion_tokens": 1000},
    )
    require(status != CostStatus.UNKNOWN and curr == "USD" and openai_cost is not None and openai_cost > 0,
            "OpenAI pricing is UNKNOWN")

    image_cost, curr, status = ProviderPricingService.estimate_cost(
        "gemini_image", "IMAGE_GENERATION", "gemini-3.1-flash-image", {}
    )
    require(status != CostStatus.UNKNOWN and curr == "USD" and image_cost is not None and 0 < image_cost <= 0.08,
            "Gemini pricing is UNKNOWN/outside reservation")

    vidu_cost, curr, status = ProviderPricingService.estimate_cost(
        "vidu", "VIDEO_GENERATION", "viduq2", {"duration_seconds": 4.0}
    )
    require(status != CostStatus.UNKNOWN and curr == "USD" and vidu_cost is not None and 0 < vidu_cost <= 0.15,
            "Vidu pricing is UNKNOWN/outside 4s 720P bound")

    vo_cost = eleven.estimate_cost(AudioGenerationParams(
        clip_id="preflight-vo", audio_type="VO", prompt="ทดสอบเสียงภาษาไทย", duration_seconds=3.0
    ))
    bgm_cost = eleven.estimate_cost(AudioGenerationParams(
        clip_id="preflight-bgm", audio_type="BGM", prompt="calm corporate instrumental", duration_seconds=10.0
    ))
    ambience_cost = eleven.estimate_cost(AudioGenerationParams(
        clip_id="preflight-ambience", audio_type="AMBIENCE", prompt="quiet office ambience", duration_seconds=3.0
    ))
    require(all(v is not None and v > 0 for v in (vo_cost, bgm_cost, ambience_cost)),
            "ElevenLabs price estimator returned UNKNOWN")

    # Verify real S3-compatible runtime without external provider calls.
    storage = get_storage_provider()
    bucket = settings.OBJECT_STORAGE_BUCKET
    key = f"wp020-live-preflight/{uuid.uuid4()}.txt"
    payload = b"orbis-live-preflight"
    ...
    # Verify DB and hard budget on an isolated UAT project. No provider event is
    # recorded, so committed live cost must remain zero.
    db = SessionLocal()
    try:
        project = Project(
            id=uuid.uuid4(),
            title="WP020 LIVE Preflight",
            description="No-paid preflight only",
            video_mode="STORY",
            status="DRAFT",
        ...
        db.add(project)
        db.commit()
        require(project.budget_limit == 1.0 and project.budget_currency == "USD", "UAT budget guard mismatch")
    ...
    # Sanitized output only: booleans and bounded numeric estimates.
    print(json.dumps({
        "status": "PREFLIGHT_PASS",
        "main_base": os.environ.get("AUTHORIZED_MAIN_SHA", ""),
