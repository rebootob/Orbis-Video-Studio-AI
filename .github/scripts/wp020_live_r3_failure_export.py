"""Enrich an R3 STOP artifact with sanitized durable creative audit metadata.

Runs only after the bounded R3 runner has already stopped. It performs no provider
...
from wp020_live_r3_contract import R3_EXECUTION_ID, sanitize_creative_audit
...
def main() -> None:
    ...
    if not summary_path.exists():
        print("R3 failure export: summary.json not present; nothing to enrich")
        return

    state = json.loads(summary_path.read_text(encoding="utf-8"))
    if state.get("execution_id") != R3_EXECUTION_ID:
        raise RuntimeError("R3 failure export execution identity mismatch")
    if state.get("status") != "STOPPED":
        print("R3 failure export: state is not STOPPED; no enrichment required")
        return

    project_id_raw = state.get("project_id")
    if not project_id_raw:
        print("R3 failure export: no project_id persisted before STOP")
        return
    ...
    if creative:
        ...
        print("R3 failure export: sanitized creative audit retained")
    else:
        print("R3 failure export: no creative audit available")
