"""P4-WP020-LIVE R4 no-paid preflight.
...
pricing/budget guards, database runtime, S3-compatible object storage, and the
immutable R4 contract. No secret values are printed and no execution fence is
written or consumed.
"""
...
from wp020_live_r4_contract import HARD_CAP_USD, MAX_PAID_CALLS, PAID_CALL_SEQUENCE, R4_EXECUTION_ID
...
def main() -> None:
    require(os.environ.get("WP020_LIVE_EXECUTION_ID") == R4_EXECUTION_ID, "R4 execution identity drift")
    require(MAX_PAID_CALLS == 6 and len(PAID_CALL_SEQUENCE) == 6, "R4 paid-call contract drift")
    ...
    require(estimated_reservation <= HARD_CAP_USD, "R4 estimated reservation exceeds USD 1.00 hard cap")

    storage = get_storage_provider()
    bucket = settings.OBJECT_STORAGE_BUCKET
    key = f"wp020-live-r4-preflight/{uuid.uuid4()}.txt"
    payload = b"orbis-live-r4-preflight"
    ...
    try:
        project = Project(
            id=uuid.uuid4(),
            title="WP020 LIVE R4 Preflight",
            description="No-paid R4 preflight only",
            video_mode="STORY",
            status="DRAFT",
