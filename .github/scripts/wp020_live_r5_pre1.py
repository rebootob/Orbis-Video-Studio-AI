"""P4-WP020-LIVE R5-PRE1 no-paid readiness / preflight.

This script MUST NOT dispatch any external Creative/Image/Video/Audio generation
request. It validates only credential presence, production adapter configuration,
pricing/budget guards, database runtime, S3-compatible object storage, and
readiness evidence. No secret values are printed and no execution fence or paid
authorization marker is written or consumed.
"""
...
CANONICAL_BASE_SHA = "8c8eb871b6d2a0522b1764c4d5e1eeae0ea1e822"
HARD_CAP_USD = 1.00
MAX_PAID_CALLS_LIMIT = 6
VIDU_CREDITS_READINESS_EVIDENCE = "2,000 credits (manual Owner-provided evidence; readiness only, not converted to USD)"
...
def main() -> None:
    ...
    # Constructor/config validation only. Never invoke generation methods here.
    ...
    estimated_reservation = sum(
        float(value)
        for value in (openai_cost, image_cost, vidu_cost, vo_cost, bgm_cost, ambience_cost)
        if value is not None
    )
    require(estimated_reservation <= HARD_CAP_USD, "R5 estimated reservation exceeds USD 1.00 hard cap")

    storage = get_storage_provider()
    bucket = settings.OBJECT_STORAGE_BUCKET
    key = f"wp020-live-r5-preflight/{uuid.uuid4()}.txt"
    payload = b"orbis-live-r5-preflight"
    ...
    try:
        project = Project(
            id=uuid.uuid4(),
            title="WP020 LIVE R5 Preflight",
            description="No-paid R5 readiness preflight only",
            video_mode="STORY",
            status="DRAFT",
        ...
        db.add(project)
        db.commit()
        require(project.budget_limit == HARD_CAP_USD, "UAT budget limit mismatch")
        ...
        require(db.query(UsageLedger).filter(UsageLedger.project_id == project.id).count() == 0, "UAT project did not start at zero ledger events")
        ...
        json.dumps(
            {
                ...
                "canonical_base_sha": os.environ.get("CANONICAL_BASE_SHA", CANONICAL_BASE_SHA),
                ...
                "pricing_estimates_usd": {
                    ...
                    "estimated_total_reservation": round(estimated_reservation, 6),
                },
                "vidu_credits_readiness": VIDU_CREDITS_READINESS_EVIDENCE,
                "budget_cap_usd": HARD_CAP_USD,
                "max_paid_calls_limit": MAX_PAID_CALLS_LIMIT,
